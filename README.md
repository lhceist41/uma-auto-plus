...
- **What about the borrowed friend card?** That one the game itself forces: the borrow resets every career, and Start Career silently does nothing while the slot is empty. So on unattended queue runs the bot has to pick one -- it takes your strong friend card when it spots it, otherwise the top row of the list. Since the game sorts trainers you follow to the top, following the trainer whose card you want makes the pick yours. You can also pick by name: type a card or character name under Run Queue Settings ("Preferred Borrow Card") and the bot borrows the first matching row.
