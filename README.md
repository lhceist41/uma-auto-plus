...
- **What about the borrowed friend card?** That one the game itself forces: the borrow resets every career, and Start Career silently does nothing while the slot is empty. So on unattended queue runs the bot has to pick one -- Smart Borrow (on by default) scans the whole borrow list and takes the best available card from a curated list of great picks, skipping any card that would duplicate a character already in your deck, and falling back to your strong friend card or the top row. Follow trainers with strong cards to give it good options, or turn Smart Borrow off to keep the plain top-row pick.
