...
- **292 character presets** -- 72 characters and outfits across 4 scenarios (URA Finale / Unity Cup / Trackblazer / Grand Concert), each pre-filling stat priorities, race plan, skill plan, event picks, and training thresholds, plus dedicated parent-farming "Legacy Farm" builds that race generated G1-dense schedules for breeding careers. A searchable trainee picker shows per-scenario recommendations, a badge telling you whether a build is proven in real runs or still research-based, and starred favorites; applying a preset sets its scenario with it. Pick a trainee, hit Start -- no manual tuning required.
...
- **Does the bot change my support deck?** It can, if **Auto-Fill Support Deck** is enabled (it is, by default). Before each career the bot presses the game's own Auto-Fill button on the deck screen, and the game then rebuilds the deck with its own selection logic -- which may **replace cards you placed yourself**, not just fill empty slots. The bot does not judge which card is better; it hands that decision to the game. On a queue this happens again for every run, not just the first. Turn Auto-Fill Support Deck off if you want your hand-built deck kept exactly as you left it (the borrowed friend card is separate -- see below).
...
- [x] Supports multiple scenarios including **URA Finale**, **Unity Cup**, **Trackblazer**, **Grand Concert**, and those in the future to come.
