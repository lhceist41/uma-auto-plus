 ...
 * status effects throughout the bot's runtime.
 */
class Trainee {
    companion object {
    ...
    /**
     * Returns the highest-aptitude enum key (the trainee's "preferred" track distance or running
     * style). Ties on aptitude break toward the lowest ordinal.
     *
    ...
    /**
     * Detects the trainee's aptitudes for a category by template-checking across the row in the
     * Details dialog identified by the [label] coordinate.
     *
    ...
    private fun updateConditions(imageUtils: CustomImageUtils) {
        ...
        for (i in 0 until 3) {
            ...
            // Identify the status type by sampling the label's background color. Sample near the corner
            // to avoid text overlap producing a wrong color match.
