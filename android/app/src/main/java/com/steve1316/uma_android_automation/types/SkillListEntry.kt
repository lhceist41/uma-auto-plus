 ...
 * @param bIsVirtual Whether this is a virtual entry: an in-place upgrade that doesn't appear in the list until all previous versions are purchased.
...
class SkillListEntry(
     ...
     *
     * For in-place skills (like status boosts) this is just the base cost. For multi-entry skills (like gold skills) the price includes all unpurchased prerequisites; since lower tiers are never
     * dearer than their upgrades, doubling the base cost is a safe upper bound for OCR validation.
     */
    ...
    /**
     * Snapshot of the OCR-detected screen price, used as a fixed baseline when computing price reductions
     * as related skills are purchased so errors don't compound.
     */
     ...
     *
     * For separate-entry (not in-place) skills, the on-screen price includes the cost of all previous unpurchased versions, so subtract the immediate predecessor to get this upgrade's standalone price.
     *
     ...
     *
     * @return The evaluation point modifier for the required [RunningStyle], or null if the skill has no such condition.
     */
     ...
     *
     * @return The evaluation point modifier for the required [TrackDistance], or null if the skill has no such condition.
     */
     ...
     *
     * @return The evaluation point modifier for the required [TrackSurface], or null if the skill has no such condition.
     */
     ...
     *
     * In-place: the next level replaces the current version in the list, so [bIsVirtual] becomes false (it now physically exists) and the price is synced to the predecessor's discount.
     *
     * Not in-place: separately-listed upgrades have their [screenPrice] reduced by the purchased skill's price, since they now only cost the "top-up" amount.
     *
     * @param entry The downgraded [SkillListEntry] that was just bought.
     */
    fun onDowngradePurchased(entry: SkillListEntry) {
        if (bIsInPlace) {
            // In-place (like status boosts): buying the previous version makes this one physically appear.
            bIsVirtual = false
            val prev: SkillListEntry? = prev
            if (prev != null) {
                // Sync price to the predecessor's discount.
                screenPrice = (skillData.cost.toDouble() * (1.0 - prev.discount)).roundToInt()
            }
        } else {
            // Separate entries (like gold skills): only the top-up difference remains to pay.
    ...
    fun onUpgradePurchased(entry: SkillListEntry) {
        ...
        // In-place chains: the upgrade replaces this one, so it becomes virtual (no longer physically present).
        if (bIsInPlace) {
            bIsVirtual = true
        }
