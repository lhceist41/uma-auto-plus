...
/**
 * Strips a trailing rank glyph that OCR misread as a letter ("O"/"x") or left as stray noise,
 * returning the base name. Glyphs are stored in the database as " <glyph>" with a leading space,
 * so the base must have the misread char removed.
 *
...
class SkillList(private val game: Game, private val campaign: Campaign) {
    ...
    fun getSkillListEntryTitle(bitmap: Bitmap? = null, debugString: String = ""): String? {
        ...
        // Strip the "Remove" prefix used for negative skill titles before glyph handling so the
        // base name is clean. The database stores the base skill name without this prefix.
        ...
        // The base skill name with any trailing misread-glyph letter removed. The database
        // stores glyphs as " <glyph>" with a leading space.
        val baseName: String = stripTrailingGlyphNoise(skillName)

        // Template match found no glyph, but OCR left a trailing letter that is really a misread
        // rank glyph: ○ reads as "O"/"0", × as "x"/"X" (◎ never survives a scan as a letter).
        // Only accept the recovery when "<base> <glyph>" is a real skill so a legitimate name
        // ending in a letter is not mangled.
        if (iconChar.isEmpty()) {
        ...
        // If a glyph was resolved (template match or recovery), re-append it with a leading
        // space to match the database formatting.
     ...
     * @param pass The 0-indexed scroll pass; scrolled passes shift the crop down to follow the settled list.
