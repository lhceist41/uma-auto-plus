 ...
 *                                 └► Team Preview (your team vs enemy, 3 races, Next button)
 ...
 * ```
 *
 * The tail of the flow (match-end back to TT home) is one advance-through-result-screens
 * state that clicks the first recognised button until the Team Trials home banner reappears.
 *
 * ## Resource accounting
 *
 * Each match costs 1 RP. Loops matches until [maxMatchesPerSession] is hit or the game
 * stops us (Team Race button disabled / opponent list empty).
 *
 * ## Configuration (via [SettingsHelper], namespace `"miscTeamTrials"`)
 *
 * - `opponentPick` (string): `"TOP"` | `"MIDDLE"` | `"BOTTOM"`. Default `"BOTTOM"`:
 *   opponents are sorted top=strongest → bottom=weakest, so bottom is the easiest fight.
 * - `maxMatchesPerSession` (int, default 5): caps per-session matches. The game already
 *   caps us at RP (5 matches per full pool); this is a secondary fuse.
 *
...
class TeamTrialsTask(game: Game) : MiscTask(game) {
    ...
    enum class TeamTrialsScreenState {
        ...
        /** Team Preview - your team vs enemy, race lineup, Next button at bottom. */
        TEAM_PREVIEW,
    ...
    private fun detectScreenState(
        ...
        // Team Trials label at top-left - catch-all for any TT screen not detected above.
        // Must come BEFORE the bottom-nav fallbacks: the Race bottom-nav tab is highlighted
        // on every TT screen, so without this ordering Team Preview / post-match screens
        // misclassify as RACE_TAB and the bot loops clicking the TT tile. Pre/post-match is
        // distinguished via [matchInProgress] (set when we commit Race!, reset on TT home).
    ...
    /**
     * Pick an opponent by the configured strategy. Opponent tile art is randomised per
     * opponent, so template matching can't identify "the bottom row" - instead click
     * coordinates are computed as ratios of the display, since the 3 rows sit at stable
     * vertical positions.
     */
     ...
     *
     * Lands back on Team Trials Home, where [handleTeamTrialsHome] starts the next match.
     */
