/** Custom data types shared across the app.
 *
 * Types used by only one class belong in that class's companion object; anything reusable
 * goes here. Types that grow complex (e.g. [GameDate]) move to their own file.
 */
...
data class BoundingBox(val x: Int, val y: Int, val w: Int, val h: Int) {
    ...
    /** Converts to an [x, y, w, h] array (no center coords). Kept for backward compatibility.
     *
...
data class SkillData(
    ...
    /** The Running Style required to activate this skill, extracted from the scraped
     * conditions. Null if the skill isn't style-specific.
     */
    ...
    /** Running Styles that best suit this skill, inferred from its activation conditions.
     * May be empty. These are only a best guess, so they grant no aptitude-based rank bonus.
     */
