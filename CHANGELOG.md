...
- **Planned-only spending for farming profiles.** Adaptive careers whose preset declares the `sparks` objective buy only what the plan says: planned skills (with their usual upgrade-chain substitution) plus whatever the existing inherited-unique and negative-skill options already allow. The broad catch-all spending that used to drain every leftover point is skipped entirely, because unplanned filler dilutes the white-spark inheritance pool a farming career exists to build -- leftover skill points at career end are intentional, not a bug. There is no automatic recovery exception: a farming preset must plan its own recovery skills, and all four farming profiles do. The profiles using this behavior: Super Creek (Blue Farm) for Unity Cup and the three Legacy Farm profiles (Daiwa Scarlet, El Condor Pasa, Air Groove). Copano Rickey URA keeps its race-reward behavior, every other preset keeps the default, and Manual mode is untouched everywhere. Older run records remain readable.
