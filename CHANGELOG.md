...
- **Choose which friend card the queue borrows.** Type a card or character name under Run Queue Settings ("Preferred Borrow Card") and the bot picks the first matching row in the Borrow Card list when it fills the empty friend slot; leave it empty for the existing pick (your strong friend card when it spots it, otherwise the top row). If the preferred pick cannot fill the slot -- for example the card would duplicate one already in your deck -- the retry falls back to the default pick, so a queue can never stall on a refused borrow.
