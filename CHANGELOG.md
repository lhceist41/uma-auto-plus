...
- **Grand Concert plays itself, start to finish.** The new career scenario ("Brighter Together Our Grand Concert") is supported end to end: training, races, training events, and skill buying run the way they do in the other scenarios, with the right stat caps (Speed 1600, Guts 1500, the rest 1300, plus whatever your inheritance sparks add on top), and the scenario's own two systems are now automated as well.
  ...
  **The Lesson shop.** The bot opens Lessons on its own, reads the three offered cards, and spends performance points on them. Songs and techniques are scored on what they actually give you: a song's immediate mastery bonus, whether its concert bonus still has enough turns left to pay off (a bonus that activates near the end is worth almost nothing), how scarce each point type is for the trainee's build, and how close the next concert is. It aims for three new songs in every concert cycle, which is the condition for a Great Success, and pushes harder as a concert approaches. Energy techniques are bought when energy is low so a turn is not lost to resting. Every purchase is transactional: the bot only commits once the confirmation dialog names the exact card it intended, and re-reads the balances afterwards, so a mis-tap or a garbled read costs a cancel rather than the wrong purchase. Visits and purchases are bounded, so a bad read cannot drain a career's points.

  **Songs come first.** A concert's result tier depends on learning three new songs in its cycle, and measured careers were missing that on roughly two of five concerts: affordable songs sat just below the buying threshold while techniques quietly drained the point pool, and one career entered its Grand finale with no new songs at all. Two rules fix it. While a cycle has fewer than three new songs, an affordable song on offer is always bought, whatever its score. And while any concert is still ahead, techniques may not spend the pool below the price of a song (sized from the game's own lesson-cost data), so the points a cycle's songs need are there when the songs appear. This guard understands point types, not just the total: once the shop has shown the next song's exact price, techniques cannot spend a point type below what that song still needs from it, while types the song does not use stay freely spendable, because a wallet can look healthy in total and still be broke in exactly the colors the next song costs. The career-end drain is unaffected, and every concert now logs how many new songs the cycle delivered.

  **The song pace now chases the full songbook, not just each concert.** Learning 16 songs by late Senior November and 18 by early Senior December unlocks a lyric event and the special version of the finale with its own gold skill reward, and the community pace for that is 3-4-4-3-3 purchased songs across the five cycles. The bot now follows that pace: the three-song concert condition stays unconditional, and the extra fourth song in the middle cycles is bought only when the purchase provably leaves enough points banked for the next cycle's three songs, so chasing the songbook can never starve a concert. It also stops buying on the final turn before a concert once the songs are secured: a song left on offer survives the concert and counts toward the next cycle's progress, while buying anything would refresh it away. Every concert entry now also logs the career's running song total.

  **Training earns the points the songs need.** Buying rules alone cannot buy a song the cycle never earned the points for, and measured careers showed exactly that: cycles missed their third song because no offer was affordable, not because the wrong card was picked. The bot now reads the Performance Points panel during training analysis -- the five balances, and which point type each facility grants this turn with its amount (the game rerolls that assignment every turn, so it is read off the screen, never assumed from the facility). While a cycle is still short of its three songs, training choices lean toward the facilities that earn the types the next song is missing, including a friendship training's double-type payout. The lean is bounded: it breaks ties and nudges close calls toward the needed points, but it never overrides a clearly better training, and points a facility would earn above a type's cap count for nothing, because the game discards them. Once the cycle has its three songs, training goes back to caring only about stats.

  **The concerts.** All five, including the Grand finale, run without you -- and when a mandatory race day lands on the very turn a concert ends (some goal chains do this), the bot hands the turn straight to its normal race handling instead of stopping on a screen it should have known. The bot recognises the pending screen, the start confirmation (ticking the Grand's "skip the cutscene" box first), the performance, both result screens, the bonuses notice, the activated-bonuses panel, and the finale's on-stage moment. At career end it spends the leftover performance points through Lessons before opening the skill screen, so points that would otherwise expire become stats and skills.

  Every tap is still gated on recognising the exact screen it belongs to. If the scenario ever shows something the bot does not know, it stops safely with the career intact and tells you what it was looking at, and pressing Start resumes the same career.

  **Queues too.** Run queues, trainee rotation, and automatic TP restore all work with Grand Concert: the queue pages the scenario carousel to Grand Concert on its own, launches each career, and when it resumes on a finished career's summary it completes that career first before starting the next. Press Start once and the queue runs the same way it does everywhere else. And every character in the roster now has a Grand Concert preset, derived from its own proven URA build: Speed-focused sprinters and milers chase the scenario's raised 1600 Speed cap, medium runners take moderate extra Speed headroom, and long-distance stayers keep their balanced weighting so training never starves the Stamina their goals need. Ten presets are proven over full unattended runs -- Taiki Shuttle, Copano Rickey, Daiwa Scarlet, Sakura Bakushin O, Super Creek, Agnes Tachyon, Biwa Hayahide, Meisho Doto, Nice Nature, and Tosen Jordan -- with two separate four-trainee rotation queues each going four for four; the rest are research-graded until their first completed careers. The roster is now 292 presets.

- **Trainee selection stopped re-reading rows it already checked.** The roster's page swipe is a short drag the list sometimes swallows, and the bot had no way to notice: it would scan the exact rows it had just read, taking up to 90 seconds to find a trainee and looking lost doing it. Every swipe is now verified by comparing the screen before and after, so the bot knows precisely how far the list really moved: a swallowed swipe is retried instead of rescanned, rows still on screen from the previous page are skipped instead of re-tapped, and the scan ends the moment the list stops moving instead of paying a full page of taps to notice the bottom. Position memory got smarter too: a scan now remembers where EVERY trainee it read is sitting, not just the one it was looking for, so the next queue's different trainee jumps straight to its tile. When the screen cannot be compared confidently, the bot falls back to exactly the old careful behavior, so the worst case is unchanged.

- **The spark auto-reroll now actually chooses between the original and the rerolled set.** After the 30 TP spend, the game shows both sets on a two-page selection screen and asks which to keep -- and until now the bot clicked through those pages blind, so whichever page the game showed first (the redrawn set) was kept regardless of what it contained. The bot now reads both pages in full (scrolling long sets; the old reader stopped at six rows and could miss sparks worth protecting), scores them against the trainee's build -- target stats first, then matching aptitude sparks, unique sparks, race sparks and planned skill sparks, with any 3-star worth keeping protected outright -- and pages over to the winner. Before committing, it checks that the final confirmation actually names the chosen set; a mismatch or an unreadable screen cancels and stops safely instead of confirming, leaving the choice for you (keeping the original is always available there, so nothing can be lost to a stop). A tie or an uncertain read always keeps the original. The run history records both sets, the kept set, and why the choice fell the way it did. Proven on live careers: a run that redrew a weaker original correctly kept the redrawn set and confirmed it unattended.

- **Recovery protection for safety-focused Long profiles.** Some Adaptive careers exist to finish reliably, not to squeeze rating. When a preset declares that intent (the `safe_completion` objective on a Long or Medium profile, or `race_reward` on Long) and the career reaches a skill-buying stop with no recovery skill owned, inherited, or about to be bought from the plan, the bot now buys one compatible recovery skill on its own: the cheapest fitting option actually visible on the skill screen at that moment, plain white before gold, respecting the profile's distance, style, and surface preferences. Nothing extra is bought when recovery is already covered -- an owned recovery, an inherited one, or one the plan buys in the same session all count, so there are no duplicates. Manual mode is completely unchanged, and profiles without a declared objective never do this. Grass Wonder (Saintly Jade Cleric) Unity Cup is the first shipped profile that arms it -- and because her own plan already carries her recovery chain, the protection there is a backstop rather than the plan.

- **Grass Wonder (Saintly Jade Cleric)**, the roster's second Grass Wonder outfit, with her own research-graded trio of presets. Same aptitudes and the same eight-goal chain as the base card, so the proven base configuration carries over; the difference is the body and the kit. Her growth leans Stamina and Wit, so training puts Stamina ahead of Power, and her skill plans buy her own recovery line (Deep Breaths into the Cooldown gold, plus A Small Breather) instead of recovery skills this outfit has no hint discount on. Two of her kit skills sit behind Potential levels the account has not reached yet, and the plans deliberately leave them out until then. With two owned outfits of one character, the trainee picker now shows two Grass Wonder rows, and the bot selects by the exact outfit banner in-game -- the base presets can never launch the Saintly Jade Cleric card, and hers can never launch the base card. Roster is now 220 presets across 72 entries.

- **Planned-only spending for farming profiles.** Adaptive careers whose preset declares the `sparks` objective buy only what the plan says during the career: planned skills (with their usual upgrade-chain substitution) plus whatever the existing inherited-unique and negative-skill options already allow. The broad catch-all spending is skipped mid-career, because unplanned filler dilutes the white-spark inheritance pool a farming career exists to build. At career end the trade-off flips -- the game simply discards whatever is left unspent -- so once the plan is exhausted there, the remaining points are spent on skills that fit the profile: right distance, right style, right surface, never negative skills the settings did not ask for. There is no automatic recovery exception: a farming preset must plan its own recovery skills, and all four farming profiles do. The profiles using this behavior: Super Creek (Blue Farm) for Unity Cup and the three Legacy Farm profiles (Daiwa Scarlet, El Condor Pasa, Air Groove). Copano Rickey URA keeps its race-reward behavior, every other preset keeps the default, and Manual mode is untouched everywhere. Older run records remain readable.

- **The bot now verifies remaining Skill Points before finishing a career (Adaptive mode).** Finishing hands back every unspent point, so after the career-end purchases the bot checks what is left against what is still buyable on the skill screen: a leftover is accepted only when no useful, affordable, profile-compatible skill remains (every skipped candidate is skipped for a recorded reason). When an affordable compatible skill is still there, or the check itself could not be completed, the bot re-runs the career-end purchase pass once -- and if it still cannot explain the remaining balance, it stops safely with a clear message before pressing Finish. The career is left open, nothing is deleted, and you can spend the points yourself and finish. Manual mode is unchanged.

- **Objective-aware skill spending for Adaptive mode (opt-in).** Presets can now declare what a career is for -- `rank` (the default, and exactly the previous behavior), `safe_completion`, `sparks`, or `race_reward` -- and Adaptive mode uses it to arm two extra spending behaviors. **Critical race**: when a mandatory goal race (read live off the Main screen's goal countdown and text -- never from packaged data, since events can redirect objectives mid-career) or a planned race is 1-2 turns away, the bot spends its skill points before the race instead of waiting for the high-water threshold. **Planned skill when affordable**: once a skill from the preset's plan has been seen on the skill screen, the bot buys it as soon as the points cover its observed price, instead of letting it sit unbought. No speculative screen-opening, no fuzzy race matching (garbled OCR simply does nothing), and both behaviors stay off for every preset that doesn't declare an objective -- and in Manual mode entirely. The run history records which race or skill triggered each purchase; older records remain readable. The Copano Rickey URA sash profile declares `race_reward` so her kit is bought before Kashiwa Kinen rather than after it.

- **Adaptive skill spending (opt-in).** A new Skill Spend Mode on the Skill Settings page: Manual (the default, exactly the current behavior -- your configured threshold keeps governing) or Adaptive, which picks the mid-career high-water threshold from an account-strength tier instead. Tiers and thresholds: New 300, Developing 350, Established 600, Endgame 1000; Auto currently uses Developing. The tier labels are a loose guide -- support quality and roster depth matter far more than the Team Rank letter, which the bot never reads. Nothing else changes: finals and career-end buying, the optimizer, presets, and the breakpoint stop all behave exactly as before, and the bot does not learn, inspect the support inventory, or touch presets. Every skill purchase in the run history now also states the threshold and tier that governed it; older run records remain readable.
...
- **An unreadable Spark list no longer ends an overnight queue.** After a Spark reroll the game shows the two sets on a small pager, and the bot reads both pages before deciding which to keep. Long sets do not fit on one screen, so it reads them in two overlapping passes and joins the halves by matching the rows they share. The second pass is captured while the list is still bouncing back from the bottom of its scroll, which can smudge a single row just enough that the two halves no longer line up, and the whole read was then thrown away. When that happened to the page the bot needed, it stopped and left the selection for you: an overnight run on 4 August finished 3 of its 11 careers and waited on that screen for the rest of the night, even though the correct page was on screen and its Sparks were plainly readable. Two things change. A read that fails to join up now takes one fresh look at the same spot before giving up, and a page that still comes back short is re-read once from the top, which is enough to fix the smudged-frame case outright. And if the reading is still incomplete, the bot no longer treats that as a reason to stop: as long as it can positively confirm the Original page is the one on screen, it keeps your original set and carries on with the queue. Keeping the original is always safe -- it is the set the career already earned, and the reroll is paid for either way -- so the only cost is that a possibly better rerolled set is passed over, which the log says plainly. The bot still stops, exactly as before, when it cannot prove which page it is looking at or cannot find the Confirm button, because those are the cases where continuing could commit the wrong set. It never presses a guessed spot on a page it could not read.

- **The Acupuncture event no longer bounces between its own two screens.** This event asks which treatment to take, and answering opens a second, two-row screen that repeats the same title and offers only declining or going back to the choices. The bot read that second screen as a shorter version of the same event, could not find the option it had been told to take, and fell back to the bottom row, which is the one that returns to the choices, so it asked the same question again. A run on 1 August spent about half a minute going round that loop three times before the event resolved on its own. The bot now recognizes the second screen for what it is and declines once, which ends the event cleanly, and says plainly in the log that the configured treatment could not be completed. Nothing else changes: this applies only to this one event, immediately after its own choice screen, and every other event keeps the same option handling as before.

- **Your race-result choice is no longer silently overridden.** Every career race ends on a Victory!, Solid Showing, or Defeat event with two options, and the settings let you pick which option to take (the shipped default takes Victory!'s second option, trading a fixed energy cost for a chance at a much smaller one). That pick never actually worked: the event matcher could only reach a one-option copy of these events that belongs to a single trainee's card, so the bot saw one option where the screen showed two and fell back to Option 1 at every single race, leaving nothing but a log warning behind. The bot now counts the option rows actually on screen before it decides which event data it is looking at, and it keeps looking until the count settles, so a half-drawn screen cannot be mistaken for a smaller event. A second option row that appears late still counts, and a reading of a single row is only believed when it holds for every look in the whole check. A two-option race result therefore reads the real two-option data and applies your pick, and the event summary in the log shows the rewards for the grade of the race just run instead of another grade's numbers. Gold City is the one trainee whose card replaces these events with her own one-option versions: hers are chosen only when a single row holds for that complete check, and every two-option or unclear race-result screen uses the same graded data as everyone else's. If the option the settings ask for is not among the rows on screen when it is time to tap, the bot re-checks the screen instead of quietly taking the first row, and says so in the log when it still cannot honor your pick. The same repair reaches every event whose data key does not spell the on-screen title exactly: "Get Well Soon!" and "Don't Overdo It!" now resolve to their real two-option data, both of Etsuko's Coverage events match their own graded copies instead of each other's, and trainee- and support-specific events that borrow words like "Victory!" or "Acupuncture" in their titles ("The Road to a Rad Victory!", "Shrine Visit", "The Applications of Acupuncture", and friends) now match their own data for their own trainee instead of someone else's.

- **Per-trainee event picks now actually apply.** Presets carry per-trainee choices for events every trainee shares, like the Acupuncture event, and those choices were dead on two counts: the shared default for the event always outranked them, and the bot attributed a shared event to an essentially random character rather than the trainee being played, so the per-trainee key almost never even matched. Shared events are now attributed to the trainee the career is actually playing, and when both a per-trainee pick and a shared default cover the same event, the per-trainee pick wins, because it is the more specific instruction. Trainees whose presets never diverge from the shared defaults behave exactly as before.

- **Race-result training events now show which finishing position they apply to.** A few events pay out differently depending on where the trainee placed, and the bundled event data left that line as an unreadable placeholder, so the Training Event Settings list showed rewards with no indication of which result earned them. The placement now reads plainly, as "1st", "2nd–5th", or "6th or worse". Only the placement lines changed; every reward value is exactly as before.

- **Skills that a scenario or an event grants are now in the bundled skill data.** A number of skills are never sold on the skill screen: the scenario or a story event hands them straight to the trainee. The bundled data left every one of them out, so whenever the bot read the trainee's own skill list it hit entries it could not identify. Those skills are now included, so that reading is complete, and a scenario-granted recovery skill now correctly counts as recovery the trainee already has (which only matters to the profiles carrying recovery protection). Nothing about buying changes: the bot only ever purchases rows the game itself puts on the skill screen, so these can never be bought. The same refresh brings the rest of the bundled game data up to date with the current Global release, adding the newest support card and the latest training events for several trainees and support cards.

- **Smart Borrow no longer gives up on a card it already found.** The bot scrolls the Borrow Card list to find the best support to borrow, then reopens the list to select it. The game reorders that list every time it opens, sorting by when each trainer last logged in, so the card can be somewhere else the second time. The second search was shorter than the first in one specific way: it only counted rows it could actually borrow, so a screenful of cards the game had greyed out as duplicates looked like the bottom of the list and ended the search on the spot, with the chosen card still sitting further down. When that happened the launch stopped with "no valid borrowed support available" even though the card was there. Both searches now cover the same ground, greyed-out rows included, and a page swipe the list swallows is retried instead of being read as the end. If the card really is gone by the time the list reopens, the bot now takes the best card the list currently offers instead of stopping, and a row it could not read clearly is never selected. When nothing borrowable exists anywhere in the list, it still stops safely and says so.

- **Mood Floor is now a visible setting, and a trainee's strict floor no longer sticks to every trainee after her.** Mood Floor is the mood the bot keeps a trainee at or above, spending a turn on recreation whenever she drops below it. A couple of trainees need a strict floor because a low mood can redirect one of their events; everyone else runs on the normal Good floor. The strict setting used to be applied by those trainees' presets and then never cleared, so once you had run one of them, every later trainee kept burning turns holding a mood she did not need, and there was no control anywhere to notice it or change it back. Picking any trainee preset now sets the floor that trainee's build actually calls for, so switching trainees, and each step of a rotation, lands on the right value regardless of who ran before. The floor also appears on the Training page as a Normal / Good / Great picker (Good is the default), so you can see what it is set to and override it after picking a preset. If your settings still carry a strict floor from before this fix, selecting any preset or rebuilding your rotation clears it.

- **Grand Concert careers no longer stop or lose points at the Complete Career screen.** Three defects conspired there, and one evening lost both a career's leftover performance points and its queue to them. First, the end-of-career Lessons drain could report "the list did not open" while the list was open in front of it: at career end the game greys out entire unaffordable cards, and a greyed top card made the whole list unrecognisable, so leftover points expired unbought. The reader now recognises greyed-out cards (they also read correctly as unaffordable), so the drain buys what the points still cover. Second, the Complete Career screen is recognised the moment it appears, whether the button art has finished fading in or not; a timing race there could previously stop the bot on that screen with every skill point unspent. Third, career-end skill buying now recovers when the skill screen misleads it: if a listed skill turns out not to be purchasable (the game refuses the buy), the bot stops retrying it, re-plans the remaining points onto skills it can actually buy, and the finish check counts such refused rows as unspendable instead of stalling the queue over money that cannot be spent. Points that genuinely cannot buy anything are finally handed back at Finish, with the reason recorded in the run history.

- **The daily reset no longer strands the bot on another app.** When the game restarts at the daily reset (or crashes mid-run), the bot relaunches it and resumes the career from Continue Career. The relaunch used to be able to close the game without reopening it, leaving the bot tapping on whatever app was behind it until it gave up -- and on a queue, the next run would then start on that dead screen too. The relaunch is now gentle (it brings the game back instead of force-closing it), it retries if the game does not return, and if the game genuinely cannot be recovered the queue pauses with a clear message instead of starting the next run on a dead screen. Your career is saved every turn, so nothing is lost.
...
- **With the auto-reroll enabled, a career that did not reroll could stop on the final "Keep this set of Sparks?" dialog.** The new spark handling recognised the two dialogs that appear after a reroll but not the ordinary one every non-rerolling career ends on, so it stopped rather than risk confirming something it could not identify. The career itself was already finished and saved; the dialog just needed a manual Confirm. That dialog is now recognised on its own terms and confirmed automatically, after the bot has read the full set it is about to keep.
...
- **The bot now remembers which sparks every career produced.** At the end of a career it reads the sparks screen -- each spark's name, its star count, and whether it's a stat, aptitude, unique, or skill spark -- and saves that alongside the career's result. This is groundwork for legacy/parent farming: the run history now knows what sparks each career actually ended with, not just its stats. (Careers where the auto-reroll spent recorded less than intended; complete records for those arrived with the spark selection support in a later update.)
...
- The reroll's spend button is now cut from the real game. The very first screenshot from the new spend-failure camera showed the Confirm Reroll dialog's button in a different green and a different size than the image the bot was matching against -- it could never have clicked it, which is why priced-positive rerolls kept getting cancelled (three lost overnight, a fourth caught on camera the same evening the camera shipped). The button image is replaced with the live pixels. And since the dialog's own note confirms you choose between the original and rerolled sparks afterwards, a successful spend now photographs the next screen too -- those captures became the material the spark selection support in a later update was built from. Until that support, the screens after a spend were new territory for the bot, so the first successful spend was worth watching.
