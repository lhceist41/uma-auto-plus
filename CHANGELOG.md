...
- **Planned-only spending for farming profiles (Phase 2B-1, opt-in).** Adaptive careers whose preset declares the `sparks` objective now buy only what the plan says: planned skills (with their usual upgrade-chain substitution) plus whatever the existing inherited-unique and negative-skill toggles already allow. The broad strategy tail that used to spend every leftover point is skipped entirely, because unplanned filler dilutes the white-spark inheritance pool a farming career exists to build -- leftover skill points are accepted, not drained. There is no automatic recovery exception: a farming preset must plan its own recovery skills, and all four migrated profiles already do. Migrated in this batch: Super Creek (Blue Farm) Unity Cup and the three Legacy Farm profiles (Daiwa Scarlet, El Condor Pasa, Air Groove). Copano Rickey URA keeps `race_reward`, every other preset keeps the default `rank` with the tail exactly as before, and Manual mode is untouched everywhere. Skill-spend records move to `trigger-v4`, adding whether the strategy tail was allowed; older records stay readable. Recovery-deficit protection is Phase 2B-2 and deferred.
