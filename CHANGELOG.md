...
- **Game UI color refresh handled.** A recent game update changed several screen banners from yellow-green to teal/mint (Skill Points header, Start Career! button, Auto-Select button). The bot's templates were against the old colors and were missing screens. All affected templates refreshed; for the Skill Points label both color variants are matched so the bot stays compatible if the game changes them again.
...
- **Bot is noticeably faster.** Several hot-path optimisations cut redundant work that was happening every Campaign loop tick — fewer screenshots, fewer template scans, less time waiting where the bot was over-cautious. Most visible during between-run navigation where post-run dialogs now fly by instead of plodding.
...
- **6+ screenshots saved per Campaign loop tick.** `checkMainScreen()` was taking 4 separate MediaProjection screenshots for what should be one shared bitmap. `executeAction()` was capturing a screenshot it didn't need for TRAIN/RACE actions. Per-turn race-day flags are now cached so we don't run the same template scans 2-3 times across handleMainScreen → decideNextAction → executeAction.
- **Navigator screen detection ~50% faster on the dominant state.** `detectScreenState()` was checking 8+ rare-state buttons before the most common state during between-run navigation (POST_RUN_RESULTS dialogs with Next/OK/Confirm/Close). Reordered so the common state is checked early, with the four advance-button checks consolidated into one short-circuit chain. Discriminating-button order preserved to avoid misclassification.
- **DialogUtils restricted to top-half scan.** Dialog title-gradient banners always render in the upper part of the screen. The full-screen scan was wasted work on every loop tick.
- **Shop coin pre-wait halved.** `updateShopCoins()` was waiting 3 seconds before a 30-try poll loop that already has its own per-try delay. Trimmed to 1 second since the poll handles settling. Saves ~2 seconds per shop visit.
- **Region restrictions on five high-traffic dialog buttons.** ButtonAutoSelect, ButtonToHome, ButtonResume, ButtonRetry, ButtonMenuBarHomeSelected all only ever appear in the bottom half of the screen but were scanning the full screen on every check. Restricted to bottomHalf.
...
- **Bot stuck on post-career skill purchase screen.** After the game's UI refresh, the bot's "Skill Points" banner template no longer matched (teal/mint banner vs old yellow-green template). This caused the post-career skill plan to silently abort with leftover SP, and the navigator would then loop forever clicking the green Confirm button on the still-visible skill screen until the queue gave up. Both layers fixed: detection now matches the new banner color, and the navigator clicks Back to escape the screen if it ever lingers.
- **Cupcake mood gain was silently lost.** When the trainee was at NORMAL mood and the bot used a Plain Cupcake, the code overwrote mood to NORMAL again instead of incrementing to GOOD — the +1 mood was thrown away. Same overwrite-vs-increment problem with Berry Sweet Cupcake. Now uses the proper Mood.increment() helper, so cupcakes actually do what they say.
- **Stray tap into the cinematic after Start Career!** On the second "Start Career!" confirmation screen, the bot was firing both the primary and the offset template clicks unconditionally. If the first click succeeded the screen transitioned away, and the second click then landed on whatever appeared next (cinematic, dialog), causing an unintended early tap and occasionally leaving the run in a weird state. Short-circuited so only the second template fires if the first didn't click.
- **Navigator could waste 3.5 minutes on oscillating screens.** The existing stuck-detection only fired when the bot saw the SAME state repeating. If detection oscillated between two known states (e.g. POST_RUN_RESULTS flickering with UNKNOWN), the counter reset every cycle and the navigator would spin until the 100-iteration timeout. New "iterations without progress" counter catches oscillation after ~30 iterations.
- **Long queues could leak memory and OOM.** Every template check was decoding a Bitmap from app assets and never recycling it. Over multi-hour queued sessions, this slowly accumulated native memory. Templates are now properly recycled in a finally block.
- **OCR CAREER fallback could misclick during seasonal events.** The home-screen OCR fallback scanned the full bottom-third of the screen for "CAREER" or "Event" text. During seasonal events with banners on the left side of the home screen reading "Event Underway!", the fallback could trigger and fire a blind tap at the CAREER button position — except the CAREER button might be moved by the event UI. OCR region tightened to only the right-half where CAREER actually lives.
