...
- **Skill plans silently aborting.** Pre-finals and career-complete skill purchases were both refusing to run because the bot couldn't recognize the skill purchase screen reliably. Each failed run was leaving 1000+ SP unspent. Fixed at three layers: the screen detection check now tolerates template drift, the skill-points OCR fallback retries with a relaxed threshold instead of giving up, and the between-runs navigator now refuses to dismiss a Career Summary screen that still has unspent skill points (preventing a stuck-from-prior-crash recovery from silently throwing the points away).
