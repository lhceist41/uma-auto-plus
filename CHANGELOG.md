...
- **Grand Concert plays itself, start to finish.** The new career scenario ("Brighter Together Our Grand Concert") is supported end to end: training, races, training events, and skill buying run the way they do in the other scenarios, with the right stat caps (Speed 1600, Guts 1500, the rest 1300, plus whatever your inheritance sparks add on top), and the scenario's own two systems are now automated as well.
  ...
  **What you still do yourself:** start the career in game (the scenario carousel is not automated yet), then press Start. Run queues, trainee rotation, and automatic TP restore remain unavailable while Grand Concert is selected, because a queue cannot launch the next career for this scenario yet. Those settings are left untouched and come back when you switch scenarios, and Home explains it whenever the scenario is picked. Grand Concert now has eleven presets: Taiki Shuttle, proven over full runs, plus a research-graded batch of ten covering every distance and both surfaces. The roster is now 231 presets.
