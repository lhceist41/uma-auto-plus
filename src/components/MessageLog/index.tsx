...
const LogItem = memo(({ item, fontSize, onLongPress, enableMessageIdDisplay }: { item: LogMessage; fontSize: number; onLongPress: (message: string) => void; enableMessageIdDisplay: boolean }) => {
    ...
    /**
     * Strip leading newlines/whitespace when a message ID prefix is shown, so text aligns with it.
...
const MessageLog = () => {
    ...
    const needsScrolling = contentHeight > viewportHeight + 10 // buffer for rounding
    const scrollThreshold = 50
    const maxScrollOffset = Math.max(0, contentHeight - viewportHeight)

    // At top / bottom of log.
...
****************************************`
    ...
    const introMessage = useMemo(() => {
        ...
        // If logs exist, Android already copied the settings string, so don't include it again.
        if (hasLogs) {
            return baseMessage
        }
    ...
    // Changing key forces a full FlashList remount so it refreshes when search is cleared.
    const listKey = useMemo(() => (searchQuery.trim().length === 0 ? "all-messages" : `search-${searchQuery}`), [searchQuery])

    // Scroll to top when data changes (search or sort). setTimeout defers until after the list updates.
    useEffect(() => {
