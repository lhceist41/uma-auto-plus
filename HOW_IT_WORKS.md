...
| Plain Cupcake | 30 | Held | 5 | Mood ≤ Normal and energy < 70%, or as the same-pass offset right after Royal Kale Juice drops mood; the last cupcake is reserved for that offset otherwise | Mood +1. Held, not a blind quick-use item: bought and kept in inventory until this trigger fires. Preferred over Berry Sweet for the Kale offset because +1 exactly cancels Kale's -1 mood |
| Berry Sweet Cupcake | 55 | Held | 5 | Same as Plain Cupcake, used for ordinary mood recovery or when Plain is unavailable/reserved | Mood +2. Same held-item policy as Plain Cupcake |
